...
class App extends Component {
  ...
  FormatDate = (dateTime) => {
    ...
    const time = dateObj.toLocaleString([], { hour: '2-digit', minute: '2-digit' });
    const timeAddHours = dateObj.getTime() + (2*60*60*1000);
    const timeConvert = new Date(timeAddHours);
    const timeEnd = timeConvert.toLocaleString([], { hour: '2-digit', minute: '2-digit' });
  ...
  return (
    <>
          ...
          <Route
            ...
            render={(props) => (
              <Home
                {...props} schedule={schedule} medication={medication} FormatDate={this.FormatDate}
            />
            )}
            exact
          />
          ...
          />
            ...
            path="/schedule"
            render={(props) => (
              <Schedule {...props} schedule={schedule} FormatDate={this.FormatDate} SortSchedule={this.SortSchedule} />
            )}
            exact
          />
                                          <Route
